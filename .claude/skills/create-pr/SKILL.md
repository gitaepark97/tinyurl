...
4. PR 제목과 본문 초안을 작성한다.
   ...
   - 본문은 변경 요약과 테스트 계획(체크리스트)을 포함한다. 예:
     ...
     - [ ] ...
     ```
   - 브랜치 이름에 이슈 번호가 있으면(`.claude/rules/branch-convention.md`의 `feature/42-...` 형식)
     본문 맨 아래에 빈 줄을 두고 `Refs #42`와 `Closes #42`를 둘 다 추가해 이슈와 연결한다.
     `Closes`가 있어야 PR의 Development 사이드바에 이슈가 "Linked issue"로 표시된다. 다만
     `feature` PR은 저장소 기본 브랜치(`main`)가 아니라 `develop`으로 병합되므로, `Closes`의
     자동 닫힘은 이 PR이 merge되는 시점이 아니라 나중에 `develop`이 `main`으로 병합(release)될
     때만 일어난다 — 그 전까지는 이슈가 링크만 되고 열린 채로 남는다는 점을 사용자에게 알려준다.
     이슈 번호가 없는 브랜치라면 둘 다 생략한다.
