...
4. PR 제목과 본문 초안을 작성한다.
   ...
   - 브랜치 이름에 이슈 번호가 있으면(`.claude/rules/branch-convention.md`의 `feature/42-...` 형식)
     본문 맨 아래에 빈 줄을 두고 `Refs #42`를 반드시 추가해 이슈와 연결한다. `Closes`/`Fixes`는
     쓰지 않는다 — `feature` PR은 `develop`으로 병합되는데, `Closes`류 키워드는 저장소 기본
     브랜치로 병합될 때만 이슈를 자동으로 닫으므로 이 저장소 구조에서는 의도치 않게 동작할 수 있다.
     이슈 번호가 없는 브랜치라면 생략한다.
